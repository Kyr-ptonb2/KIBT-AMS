...
/// Normalises free-text business type entries so that variants differing only
/// in case or stray whitespace (e.g. "sole proprietor", "SOLE PROPRIETOR",
/// "Sole  Proprietor") collapse into one consistent value ("Sole Proprietor").
/// Returns None for empty/whitespace-only input so it stores as NULL, same as before.
pub fn normalize_business_type(raw: &str) -> Option<String> {
...
pub fn get_participants(
    ...
    let mut sql = String::from(r#"
        SELECT p.id, p.event_id, p.name, p.business_type, p.age_category,
               p.gender, p.phone, p.consent, p.location,
               COALESCE(NULLIF(TRIM(p.region), ''), e.region) AS region,
               p.extra_fields, p.id_number, p.added_at
        FROM participants p
    ...
    if let Some(ref v) = filter.region          { sql.push_str(" AND COALESCE(NULLIF(TRIM(p.region), ''), e.region) = ?"); bind.push(v.clone()); }
    ...
    let rows = stmt
        .query_map(rusqlite::params_from_iter(bind.iter()), |row| {
            Ok(Participant {
                ...
                location: row.get(8)?, region: row.get(9)?,
                extra_fields: row.get(10)?, id_number: row.get(11)?, added_at: row.get(12)?,
            })
        })
...
pub fn save_participants(
    ...
    for row in &rows {
        ...
        let business_type = row.business_type.as_deref()
            .and_then(normalize_business_type);
        tx.execute(
            ...
            params![id, event_id, session_id, row.name.trim(), business_type, row.age_category,
...
pub fn update_participant(
    ...
    let conn = open(&state.0).map_err(|e| e.to_string())?;
    let business_type = input.business_type.as_deref()
        .and_then(normalize_business_type);
    let rows = conn.execute(
        r#"UPDATE participants SET name=?1, business_type=?2, age_category=?3,
           ...
           WHERE id=?10"#,
        params![input.name.trim(), business_type, input.age_category,
    ...
    for row in &rows {
        ...
        tx.execute(
            ...
            params![
                ...
                business_type, row.age_category, row.gender, row.phone,
