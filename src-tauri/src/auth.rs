// auth.rs — Authentication, session management, user CRUD, all with audit logging.
...
pub struct User {
    ...
    pub must_change_password: bool, pub created_by: Option<String>,
...
pub struct LoginResult {
    pub success: bool, pub user: Option<SessionUser>, pub error: Option<String>,
}
...
pub fn init_auth(app_data_dir: &std::path::Path) -> Result<()> {
    ...
    conn.execute_batch(r#"
        CREATE TABLE IF NOT EXISTS users (
            ...
            full_name TEXT, email TEXT, phone TEXT, id_number TEXT,
            must_change_password INTEGER NOT NULL DEFAULT 1,
            created_by TEXT, created_at TEXT NOT NULL, last_login TEXT
        );
    "#)?;
    ...
    if count == 0 {
        ...
        conn.execute(
            "INSERT INTO users (id,username,password_hash,role,must_change_password,created_at)
             VALUES (?1,'admin',?2,'super_admin',1,?3)",
            params![id, hash, now],
        )?;
        eprintln!("[auth] Default super_admin created. Login: admin / Kibt@2024");
    }
    Ok(())
}
...
pub fn login(
    ...
    let row = conn.query_row(
        "SELECT id,username,password_hash,role,full_name,must_change_password
         FROM users WHERE username=?1 COLLATE NOCASE",
        params![input.username.trim()],
        |r| Ok((r.get::<_,String>(0)?,r.get::<_,String>(1)?,r.get::<_,String>(2)?,
                r.get::<_,String>(3)?,r.get::<_,Option<String>>(4)?,r.get::<_,bool>(5)?)),
    );

    let (id, username, hash, role, full_name, must_change) = match row {
        Ok(r) => r,
        Err(_) => {
    ...
    write_log(&state.0, Some(&id), Some(&username), "auth.login", "auth", None, None, None);

    let session = SessionUser { id, username, role, full_name, must_change_password: must_change };
    *auth.0.lock().unwrap() = Some(session.clone());
    Ok(LoginResult { success: true, user: Some(session), error: None })
}
...
pub fn setup_profile(
    ...
    let hash = bcrypt::hash(&input.new_password, bcrypt::DEFAULT_COST).map_err(|e| e.to_string())?;
    conn.execute(
        "UPDATE users SET username=?1,password_hash=?2,full_name=?3,email=?4,
         phone=?5,id_number=?6,must_change_password=0 WHERE id=?7",
        params![input.new_username.trim(), hash, input.full_name.trim(),
                input.email.trim(), input.phone.trim(), input.id_number.trim(), session.id],
    ).map_err(|e| e.to_string())?;

    let updated = SessionUser {
        id: session.id.clone(), username: input.new_username.trim().to_string(),
        role: session.role, full_name: Some(input.full_name.trim().to_string()),
        must_change_password: false,
    };

    write_log(&state.0, Some(&updated.id), Some(&updated.username),
        "auth.profile_setup", "auth", None, Some("Profile configured for first time"), None);
...
pub fn get_users(
    ...
    let mut stmt = conn.prepare(
        "SELECT id,username,role,full_name,email,phone,id_number,
    ...
    let users = stmt.query_map([], |r| Ok(User {
        ...
        created_by: r.get(8)?, created_at: r.get(9)?, last_login: r.get(10)?,
    })).map_err(|e| e.to_string())?
...
pub fn create_user(
    ...
    conn.execute(
        "INSERT INTO users (id,username,password_hash,role,full_name,email,phone,id_number,
    ...
    write_log(&state.0, Some(&session.id), Some(&session.username),
        ...
        Some(&format!("{} (role: {})", input.username.trim(), input.role)),
        None);

    Ok(User {
        id: id.clone(), username: input.username.trim().to_string(),
        role: input.role, full_name: input.full_name,
        email: input.email, phone: input.phone, id_number: input.id_number,
        must_change_password: true, created_by: Some(session.username),
...
pub fn set_user_role(
    ...
    if rows > 0 {
        write_log(&state.0, Some(&session.id), Some(&session.username),
            "user.role_change", "user", Some(&user_id),
            Some(&format!("set to: {}", role)), None);
    }
    Ok(rows > 0)
}
